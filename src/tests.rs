...
mod tests {
    ...
    use crate::handlers::{login, register, refresh};
    ...
    async fn test_refresh_endpoint() {
        ...
        let jwt: serde_json::Value =
            serde_json::from_slice(&login_resp_body).expect("Invalid JSON");
        let refresh_token = jwt.get("refresh")
